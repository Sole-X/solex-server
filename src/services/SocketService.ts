...
export class SocketService {
  private io = require('socket.io-emitter')({ host: process.env.REDIS_HOST, port:process.env.REDIS_PORT });
  ...
  async bridge(hash,depositId,txHash,type){
    ...
    if(type=='vault'){
      var mintIndex = bridgeTx.map(function(item) { return item.type; }).indexOf(this.constant.STATUS.TOKEN.MINT);
      if(mintIndex>-1){
        result.mint['txHash'] = bridgeTx[mintIndex].txHash;
        result.mint['status'] = bridgeTx[mintIndex].status;        
      }
    }else if(type=="mint"){
      var vaultIndex = bridgeTx.map(function(item) { return item.type; }).indexOf(this.constant.STATUS.TOKEN.VAULT);
      if(vaultIndex>-1){
        result.vault['txHash'] = bridgeTx[vaultIndex].txHash;
        result.vault['status'] = bridgeTx[vaultIndex].status;        
      }      
    }else if(type=="burn"){
      var sendIndex = bridgeTx.map(function(item) { return item.type; }).indexOf(this.constant.STATUS.TOKEN.SEND);
      if(sendIndex>-1){
        result.send['txHash'] = bridgeTx[sendIndex].txHash;
        result.send['status'] = bridgeTx[sendIndex].status;        
      }            
    }else if(type=="send"){
      var burnIndex = bridgeTx.map(function(item) { return item.type; }).indexOf(this.constant.STATUS.TOKEN.BURN);
      if(burnIndex>-1){
