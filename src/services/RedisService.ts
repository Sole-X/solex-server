...
export class RedisService {
  ...
  public async set (key: string, value: any, expireTime: number = 0): Promise<any> {
    ...
    if (expireTime > 0) {
      return promisify(this._client.setex).bind(this._client)(key, expireTime, value).then(result => {
        return Promise.resolve(result ? result : undefined)
      }).catch(err => {
        console.error(err)
        return undefined
      ...
      return promisify(this._client.set).bind(this._client)(key, value).catch(err => {
  ...
  public async sismember (key: string,value:string): Promise<any> {
    ...
    return promisify(this._client.sismember).bind(this._client)(key,value)
      .then(result => {
        return Promise.resolve(result ? result : undefined)
      }).catch(err => {
        this.logger.error(err)
  ...
  public async smembers (key: string): Promise<any> {
    ...
    return promisify(this._client.smembers).bind(this._client)(key)
      .then(result => {
        return Promise.resolve(result ? result : undefined)
      }).catch(err => {
        this.logger.error(err)
