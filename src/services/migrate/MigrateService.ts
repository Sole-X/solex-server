...
export class MigrateService {
  ...
  constructor(@Inject('logger') private logger, @Inject('RedisService') private redisService, private syncService) {
  ...
  public async migrateBlock(version, range = null, startBlock = null) {
    ...
    var currentBlockNo = !startBlock ? await this.redisService.get('currentBlockNo') : startBlock;
    var term = 1000;
    ...
    if (range != null && range !== '') {
      ...
      for (let blockNumber = endRange, sleepIdx = 1; blockNumber >= startRange; blockNumber--) {
      ...
      while (currentBlockNo > 1) {
        ...
        var blockIdSet = await this.getBlockIds(currentBlockNo - term, currentBlockNo);
  ...
  public async getCurrentMigrationVersion() {
    ...
    if (Container.get('migrationVersion') != migrationInfo.migrationVersion) {
      ...
      await this.migrateBlock(migrationInfo.migrationVersion, migrationInfo.migrationRange);
      ...
      if (migrateChk) {
        this.migrateBlock(migrationInfo.migrationVersion, null, migrateChk.blockNumber);
      }
    }
  }
