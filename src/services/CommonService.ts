...
export class CommonService {
  ...
  dprec(str, d, force = false) { // 강제로 소수점 이하 d자리 출력
    if (!force && parseFloat(str) === 0) {
      return '0';
    }

    let dot = str.indexOf('.'); if (dot === -1) dot = str.length;
    let num = str.rsubstr(0, dot), dec = str.substr(dot + 1);
  ...
  async convertStatusStr(items, type, target = "STATUS", columnName = "status") {
    for (let i = 0; i < items.length; i++) {
      const key = Object.keys(this.constant[target][type]).find(key => this.constant[target][type][key] === items[i][columnName]);
      ...
      where[priceName] = Raw((alias) => {
        ...
        for (let i = 0; i < priceArr.length; i++) {
          ...
          if (priceArr[i].indexOf("-") > -1) {
            ...
            sql += " ( " + this.convertMinUnit(nowCurrency, price[0]) + " <= " + alias + " and " + this.convertMinUnit(nowCurrency, price[1]) + " >= " + alias + ") OR";
  ...
  constantToStr(datas, target) {
    ...
    datas = datas.reduce((result, data: any) => {
      if ('status' in datas && datas.status) {
        const status = Object.keys(this.constant.STATUS[target]).find(key => this.constant.STATUS[target][key] === data.status);
        data['statusStr'] = status;
      }
      if ('type' in datas && datas.status) {
        const type = Object.keys(this.constant.TYPE[target]).find(key => this.constant.TYPE[target][key] === data.type);
        data['typeStr'] = type;
      }
      if ('eventType' in datas && datas.status) {
        const eventType = Object.keys(this.constant.TYPE.EVENT).find(key => this.constant.TYPE.EVENT[key] === data.eventType);
  ...
  async bindAccountDesc(datas: any[], keys = []) {
    ...
    const accountArr = await Account.find({ select: ['accountAddress', 'username', 'profile', 'display'], where: { accountAddress: In(accountIds) } });
