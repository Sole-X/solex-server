...
export class Sync{
  ...
  public async blockSync(blockNo){
    ...
    return this.nodeService.getBlockWithConsensusInfo(blockNo).then(async (currBlock)=>{
  ...
  public async txDataSync(blockNo,transactions,blockDate){
    ...
    for (const transaction of transactions) {
      ...
      if(this.abiService.checkTradeContract(transaction.to)){
        ...
        this.addCallback(blockNo,()=>{this.socketService.resultTx(transaction.transactionHash,(transaction.status=="0x1")?true:false)});
