...
export class EventService {
  ...
  public async handler(blockNo, txIndex, eventIndex, log, blockDate, callbackQueue) {
    ...
    if (this.abiService.checkTradeContract(log.address)) {
      ...
      if (log.address == contractAddress['AuctionContract']) {
        return await this.auctionService.handler(blockNo, eventIndex, topicHash, log, blockDate, callbackQueue);
      } else if (log.address == contractAddress['ReserveContract']) {
        return await this.reserveService.handler(blockNo, topicHash, log, blockDate, callbackQueue);
      } else if (log.address == contractAddress['SellOfferContract']) {
        return await this.sellService.handler(blockNo, topicHash, log, blockDate);
      } else if (log.address == contractAddress['BuyOfferContract']) {
        return await this.buyService.handler(blockNo, topicHash, log, blockDate);
      } else if (log.address == contractAddress['StakeContract']) {
        return await this.stakeService.handler(blockNo, topicHash, log, blockDate);
      } else if (log.address == contractAddress['KlayMintContract']) {
        return await this.klayMintService.handler(blockNo, topicHash, log, blockDate, callbackQueue);
      }
    }
  }
}
