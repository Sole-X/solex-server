...
export class EthVaultService {
  ...
  async deposit(blockNo,params,txHash,blockDate,data,callbackQueue){
    ...
    const tokenInfo = await TokenInfo.findOne({where:{ethAddress:params.token}});
    ...
    this.addCallback(blockNo,()=>{this.socketService.bridge(data[1],params.depositId,txHash,'vault')},callbackQueue);
  ...
  async depositNFT(blockNo,params,txHash,blockDate,data,callbackQueue){
    ...
    const toOther = (params.fromAddr != data[0])? true:false;
    const activity = await Activity.findOne({tradeId:data[1],accountAddress:params.fromAddr,bridgeId:params.depositId});
    const tokenInfo = await NftInfo.findOne({where:{ethAddress:params.token}});
  ...
  async withdraw(blockNo,params,txHash,blockDate,data,callbackQueue){    
    ...
    const activity = await Activity.findOne({where:{tradeId:data[1],bridgeId:params.uints[2]}});
    const tokenInfo = await TokenInfo.findOne({where:{ethAddress:params.token}});
    const tokenAddr = (tokenInfo)? tokenInfo.tokenAddress:params.token;
    ...
    this.addCallback(blockNo,()=>{this.socketService.bridge(data[1],params.uints[2],txHash,'send')},callbackQueue);
