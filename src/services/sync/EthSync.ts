...
export class EthSync {
  ...
  public async blockSync(blockNo) {
    return this.nodeService
      .getBlockWithConsensusInfo(blockNo, "eth")
  ...
  public async txDataSync(blockNo, transactions, blockDate) {
    for (const transaction of transactions) {
      ...
      if (
        transaction.to &&
        transaction.to.toLowerCase() == this.contractAddress["EthVaultContract"]
      ) {
  ...
  public async eventDataSync(blockNumber, txIndex, hash, blockDate) {
    ...
    for (const log of receipt.logs) {
      ...
      await this.vaultService.handler(
        blockNumber,
        topicHash,
        log,
        blockDate,
