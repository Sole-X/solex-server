...
export class BulkService {
      ...
      if(this.upsertQueue.size>0 && this.upsertQueue.has(idKey)){
        ...
        for (var entry of upsertBulk.entries()) {
          ...
          for(let i=0;i<value.length;i++){
            ...
            .into(tableName)
            .values(value[i].data)
            .orUpdate({ conflict_target: value[i].conflict_target, overwrite: value[i].overwrite })
            .updateEntity(false)
            .execute();
          }
        }
  ...
  async rankSyncInBulk(bulkService,blockNo){
    const ownerCntresult = await getRepository(NftItem).query("SELECT tokenAddress,count(tokenAddress) as ownerCnt from "+
      " (select tokenAddress,ownerAddress FROM nft_item WHERE status != '"+this.constant.STATUS.NFT.WITHDRAW+"' group by tokenAddress, ownerAddress ) "+
      " AS T2 group by tokenAddress");
    const nftCntResult = await getRepository(NftItem).query("SELECT tokenAddress,count(*) AS cnt "+
