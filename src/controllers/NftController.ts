...
exports.saleSearch = async function (req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    if (order.indexOf('PARTICIPANT') < 0 && status.indexOf('SELL') > -1)
      statusArr.push(constant.TYPE.SALE.DIRECT_SELL, constant.TYPE.SALE.NEGOTIABLE_SELL);
    if (!status || status.indexOf('AUCTION') > -1)
      statusArr.push(constant.TYPE.SALE.NORMAL_AUCTION, constant.TYPE.SALE.INSTANT_AUCTION);
...
exports.matchSearch = async function (req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    accounts = await Account.find({
      ...
      where: [{ accountAddress: Like('%' + search + '%') }, { username: Like('%' + search + '%') }],
      take: limit,
    });
