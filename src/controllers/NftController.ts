...
//아이템 검색 (Sale)
exports.saleSearch = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  try {
    ...
    var statusArr = [];
    if (order.indexOf("PARTICIPANT") < 0 && status.indexOf("SELL") > -1)
      statusArr.push(
        constant.TYPE.SALE.DIRECT_SELL,
        constant.TYPE.SALE.NEGOTIABLE_SELL
      );
    if (!status || status.indexOf("AUCTION") > -1)
      statusArr.push(
    ...
    if (lifeStatus == "START") {
      ...
      where["status"] = In([
        constant.STATUS.SALE.START,
        constant.STATUS.SALE.DONE,
      ]);
    }
...
//아이템/컬렉션/계정 일치 검색
exports.matchSearch = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
//특정 아이템 정보
exports.getCollectionInfo = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  try {
    ...
    const publisherCnt = await NftItem.count({
...
exports.getNft = async function (
  ...
  try {
    ...
    if (!result && buyId && buyId != "") {
      ...
      buyInfo = (
        await nftService.bindInfo([buyInfo], ["like"], {
      ...
      result = (
        await nftService.bindInfo([result], ["trade", "like", "buyInfo"], {
          connectAddr: req.query.connectAddr,
        })
      )[0];
      const key = Object.keys(constant.STATUS.NFT).find(
        (key) => constant.STATUS.NFT[key] === result.status
      );
      ...
      const createActivity = await Activity.findOne({
        ...
        where: {
          ...
          eventType: constant.TYPE.EVENT.NFT,
          status: 1,
        },
        order: { createdAt: "ASC" },
      });
      if (createActivity) {
        result["creator"] = await NftInfo.findOne(
          createActivity.accountAddress
        );
      } else {
        result["creator"] = "";
      }
...
exports.latest = async function (
  ...
  try {
    var result = await getRepository(NftItem).find({
      order: { createdAt: "DESC" },
      take: 10,
      relations: ["desc"],
    });
    result = await nftService.bindInfo(result, ["trade", "like", "buyInfo"], {
      connectAddr: req.query.connectAddr,
    });
...
exports.like = async function (
  ...
  try {
    ...
    const nftLiked: NftLiked = await getRepository(NftLiked).findOne({
      where: { accountAddress: accountAddr, tradeId: tradeId },
    });
    if (!nftLiked) {
      await getRepository(NftLiked).insert({
...
//찜 해제
exports.deleteLike = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  try {
    ...
    if (nftLiked) {
      await getRepository(NftLiked).delete({
        accountAddress: accountAddr,
        tradeId: tradeId,
      });
      if (trade.liked > 0)
        await getRepository(type).decrement({ id: tradeId }, "liked", 1);
    }
...
//찜 목록
exports.getLike = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  try {
    ...
    var nftLikeds = await NftLiked.pagination(
      req.query.page,
      req.query.limit,
      { accountAddress: accountAddr },
      { createdAt: "DESC" },
...
//찜 많은 눌린 아이템 목록
exports.mostLike = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
//체인별 최신 아이템 목록
exports.latestChain = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
//address 별 최근 본 아이템
exports.recentViewList = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  return redisServer
    ...
    .then(async (result) => {
      ...
      nfts = result.reduce((result, item: any) => {
        if (item in nftInfos) {
          if ("sellerAddress" in nftInfos[item]) {
            ...
            if (
              item.type == constant.TYPE.SALE.NORMAL_AUCTION ||
              item.type == constant.TYPE.SALE.INSTANT_AUCTION
            ) {
      ...
      nfts = await nftService.bindInfo(nfts, ["like", "buyInfo"], {
        connectAddr: req.query.connectAddr,
      });
...
//최근 본 아이템
exports.recentView = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
...
//거래내역 & 가격 변동내역
exports.getNftHistory = async function (
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  try {
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      if (
        ...
        item.usdPrice = await commonService.convertUsdPrice(
          item.currency,
          item.amount
        );
      }
