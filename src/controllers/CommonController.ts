  ...
  try {
    ...
    if (nftCnt && Object.keys(nftCnt).length > 1) {
      ...
      for (let i = 0; i < nftCate.length; i++) {
        const cateAddrArr = await Category.find({
          where: { category: nftCate[i], type: constant.TYPE.CATEGORY.NFT },
        });
      ...
      const nftInfos = await NftInfo.find({
        status: constant.STATUS.NFT_INFO.USE,
      });
      for (let i = 0; i < nftInfos.length; i++) {
        query +=
          "SUM( CASE WHEN tokenAddress = '" +
      ...
      const tokenInfos = await TokenInfo.find({
        status: constant.STATUS.TOKEN_INFO.USE,
      });
