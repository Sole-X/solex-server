...
exports.getActivity = async function (req:Request, res:Response, next:NextFunction) {
  ...
  try {
    ...
    data.items = await commonService.convertStatusStr(data.items,"EVENT",'TYPE','eventType')
    data.items = await commonService.bindAccountDesc(data.items,"EVENT",'TYPE','eventType')
...
exports.getRanking = async function (req:Request, res:Response, next:NextFunction) {
  const commonService:any = Container.get('CommonService');
