...
exports.getSell = async function (req, res, next) {
  ...
  try {
    const result:any = await Sale.pagination(req.params.page, req.params.limit, where, order,relation);

    if(result){
      result.items = await commonService.convertStatusStr(result.items,'SELL');

      for(let i=0; i<result.items.length; i++){
        if(result.items[i].negos) result.items[i].negos = await commonService.convertStatusStr(result.items[i].negos,"NEGO");
      }
    }
...
exports.getNegos = async function (req, res, next) {
  ...
  try {
    const result = await SellNego.pagination(req.params.page, req.params.limit, where,order);
    return res.status(200).json( result );
  } catch (e) {
    return next(e);
  }
...
exports.declineNego = async function (req, res, next) {
  ...
  try {
    const [ result, signAddress, v, r, s ] = await nodeService.checkSignAddress(connectAddr,hashType,msg,signHash);

    const sell = await Sale.findOne(sellId);

    if(!result) throw Error("INVALID OWNER");
    if(!sell) throw Error("NO SELL");
    if(sell.ownerAddress.toLowerCase() != connectAddr.toLowerCase()) throw Error("INVALID OWNER");
