...
exports.search = async function (req, res, next) {
  ...
  try {
    var where = await commonService.makeWhereFromReq({ status:req.query.status });
...
exports.getSale = async function (req, res, next) {
  ...
  try {
    var result = await Sale.findOne({where:{id:tradeId}, relations:['bids','negos']});
    if(!result) throw Error("NO SALE");

    //?connectAddr=0x2bfe852660947d88d5156e5ae3e387e351ea2cb9
    result = (await nftService.bindInfo([result],['like'],{connectAddr:req.query.connectAddr}))[0];
