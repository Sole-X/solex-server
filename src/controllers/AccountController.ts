...
exports.getTotalBalance = async function (req, res, next){
  ...
  const balances = await TokenBalance.find({where:{accountAddress:accountAddr,tokenAddress:In(tokenArr)}});
  const count = await NftItem.count({ownerAddress:accountAddr,status:Not(constant.STATUS.TOKEN.WITHDRAW)})
  const sellCount = await NftItem.count({ownerAddress:accountAddr,status:constant.STATUS.TOKEN.SELL})
  const auctionCount = await NftItem.count({ownerAddress:accountAddr,status:constant.STATUS.TOKEN.AUCTION})
  const nftActivityCount = await Activity.count({accountAddress:accountAddr,eventType:Not(constant.TYPE.EVENT.TOKEN)})
  const tokenActivityCount = await Activity.count({accountAddress:accountAddr,eventType:constant.TYPE.EVENT.TOKEN})  
  ...
  try {
    return res.status(200).json({ tokens: balances,
      nftCnt:count,
...
exports.getNfts = async function (req, res, next){
  ...
  try {
    var where = await commonService.makeWhereFromReq({ ownerAddress:accountAddr });
    ...
    if(nfts.items.length>0) nfts.items = await nftService.bindInfo(nfts.items,['trade','like','buyInfo'],{connectAddr:req.query.connectAddr});
...
exports.getActivites = async function (req, res, next){
  ...
  try {
    const nftInfos = await NftInfo.find({
      ...
      where:[
      {name:Like("%"+tokenName+"%")},
      {symbol:Like("%"+tokenName+"%")}
      ]}
    )
    nftInfos.forEach(nftInfo=>{ collections += ","+nftInfo.tokenAddress});
    ...
    activites.items = activites.items.reduce((result,item:any)=>{
      ...
      const key = Object.keys(constant.STATUS[eventType]).find(key => constant.STATUS[eventType][key] === item.status);
      item['statusStr'] = key
      result.push(item)
      return result;
    },[]);
...
exports.getStaking = async function (req, res, next){
  ...
  const where = await commonService.makeWhereFromReq({ accountAddress:accountAddr });

  var activites = await StakeActivity.pagination(page, limit, where,{updatedAt:"DESC"});
...
exports.editProfile = async function (req, res, next){
  ...
  try {
    ...
    if(req.body.file && req.body.file.toString().indexOf('base64')>-1){
      ...
      if((imageBuffer.length / 1e+6)>10) throw Error("INVALID FILE SIZE") 
...
exports.setAgreement = async function (req, res, next){
  ...
  try {
    ...
    if(!result){
      await Agreement.insert({accountAddress:accountAddr,agreementCate:cate,status:status});
    }else{
