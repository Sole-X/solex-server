  ...
  try {
    while (true) {
      ...
      for (let i = 0; i < dataArr.length; i++) {
        const hash =
          ethUtil.randomHex(28) +
          ethUtil.toHex(Math.floor(Date.now() / 1000)).slice(2);
        const executeAbi = abiService.getFunctionAbi(
          "auction-abi",
          "closeAuction"
        );
        const encoded = abiCoder.encodeFunctionCall(executeAbi, [
          hash,
          dataArr[i].id,
        ]);
        ...
        try {
          const result = await caver.kas.wallet.requestSmartContractExecution(
            tx
          );
