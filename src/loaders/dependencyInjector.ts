...
const kasCaver = new CaverExtKAS(
  process.env.KAS_NETWORK,
...
export default async () => {
  try {
    ...
    for (let objKey of Object.keys(contractAddress)) {
      if (contractAddress[objKey].indexOf("0x") > -1)
        contractAddress[objKey] = contractAddress[objKey].toLowerCase();
    }

    if (process.env.BLOCK_RPC_USING == "true") {
      Container.set(
        "caverClient",
        new Caver(process.env.BLOCK_RPC || "http://klaytn.dxm.kr:8551")
      );
    } else {
    ...
    Container.set(
      "migrationVersion",
      (await Variable.findOne({ key: "migrationVersion" })).value
    );
